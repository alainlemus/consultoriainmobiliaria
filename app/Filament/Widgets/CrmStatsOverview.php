...
class CrmStatsOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make('Cerrados este mes', $cerradosMes)
                ->description(now()->translatedFormat('F Y'))
                ->descriptionIcon('heroicon-m-check-badge')
                ->color('success'),

            Stat::make('Prospectos nuevos', $prospectoNuevos)
                ->description('Sin atender del sitio web')
                ->descriptionIcon('heroicon-m-user-plus')
                ->color('warning'),

            Stat::make('Honorarios estimados', '$' . number_format($ingresosEstimados, 0, '.', ','))
                ->description('Expedientes activos + cerrados')
                ->descriptionIcon('heroicon-m-banknotes')
                ->color('success'),

            Stat::make('Comisiones por pagar', '$' . number_format($comisionesPendientes, 0, '.', ','))
